...
> RunPod RTX 4090 기반 AI 도시 시뮬레이터
...
| LLM Runtime | vLLM OpenAI-compatible API | 시민 reflection, God Mode 자연어 명령 해석 경로. 실패 시 deterministic fallback 유지 |
...
| Persistence | JSON-backed learning store, memory fallback | demo adaptation 상태와 이벤트 경험을 가볍게 유지 |
| Runtime | RunPod RTX 4090 direct processes | vLLM, vision, orchestrator, simulation, STT/traffic AI를 Docker 없이 실행 |
...
브라우저는 3D 렌더링과 인터랙션에 집중하고, 무거운 AI와 시뮬레이션은 RunPod RTX 4090 서버가 담당합니다.
...
차량 시점과 vision detection 결과를 보여줍니다. real YOLO 경로가 활성화되어 있으면 RunPod 4090 기반 vision임을 표시합니다.
